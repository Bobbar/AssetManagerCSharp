    ...
    public static class MunisFunctions //Be warned. This whole class is a horrible bastard...
    {
        ...
        public static async void NewOrgObjView(string org, string obj, string fy, ExtendedForm parentForm)
        {
            try
            {
                ...
                string glMasterQry = "Select TOP " + maxResults + " " + glColumns + "FROM glmaster";
                ...
                if (!string.IsNullOrEmpty(obj)) //Show Rollup info for Object
                {
                    ...
                    string rollUpByCodeQry = "SELECT TOP " + maxResults + " * FROM gl_budget_rollup WHERE a_rollup_code = '" + rollUpCode + "'";
                    string budgetQry = "SELECT TOP " + maxResults + " a_projection_no,a_org,a_object,db_line,db_bud_desc_line1,db_bud_reason_desc,db_bud_req_qty5,db_bud_unit_cost,db_bud_req_amt5,a_account_id FROM gl_budget_detail_2"; // WHERE a_projection_no='" & FY & "' AND a_org='" & Org & "' AND a_object='" & Obj & "'"
                ...
                {
                    string rollUpAllQry = "SELECT TOP " + maxResults + " * FROM gl_budget_rollup";
        ...
        private static async void NewMunisEmployeeSearch(string name, ExtendedForm parentForm)
        {
            try
            {
                ...
                string query = "SELECT TOP " + maxResults + " " + columns + @"
...
INNER JOIN pr_employee_master m on e.e_supervisor = m.a_employee_number";
        ...
        public static async void NewMunisPOSearch(string po, ExtendedForm parentForm)
        {
            try
            {
                ...
                string query = "SELECT TOP " + maxResults + @" pohd_pur_no, pohd_fsc_yr, pohd_req_no, pohd_gen_cm, pohd_buy_id, pohd_pre_dt, pohd_exp_dt, pohd_sta_cd, pohd_vnd_cd, pohd_dep_cd, pohd_shp_cd, pohd_tot_amt, pohd_serial
FROM poheader";
        ...
        private static async Task<DataTable> GetReqHeaderFromReqNum(string reqNumber, string fiscalYr)
        {
            ...
            string query = "SELECT TOP " + maxResults + " * FROM rqheader";
        ...
        private static async Task<DataTable> GetReqLineItemsFromReqNum(string reqNumber, string fiscalYr)
        {
            ...
            string query = "SELECT TOP " + maxResults + @" dbo.rq_gl_info.rg_fiscal_year, dbo.rq_gl_info.a_requisition_no, dbo.rq_gl_info.rg_org, dbo.rq_gl_info.rg_object, dbo.rq_gl_info.a_org_description, dbo.rq_gl_info.a_object_desc,
